...
def compute(comm_list):
    ...
    df6 = pd.DataFrame(comm_list)
    df6.columns = ["text"]
    # print(df6)
    df6["text"] = df6["text"].str.lower()
    df6["text"] = df6["text"].str.replace("[^\w\s]", "")
    df6["text"] = df6["text"].str.replace("\d+", "")
    df6.dropna(subset=["text"], inplace=True)
    df6 = df6[~df6.isin([" ", "  "]).any(axis=1)]
    stop = set(stopwords.words("english"))
    ps = SnowballStemmer("english")
    df6["text"] = df6["text"].apply(
...
def main():
    ...
    decoded_id = unquote(sys.argv[1], encoding="utf-8")
